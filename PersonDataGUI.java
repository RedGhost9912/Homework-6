...
public class PersonDataGUI {
	...
	public static Person[] firstList;
	...
	public static void getfirstList(){
		firstList = new Person[8];
		firstList = FileReader.readPeople();
	}
	...
	public void createAndShowGUI() {
		...
		JButton buttonReset = new JButton("Възстанви");
		...
		buttonReset.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				personDataModel = new PersonDataModel(firstList);
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent MousE) {
				...
				if (MousE.getClickCount() == 2) {
					...
					int Egn = (int) (table.getValueAt(row, 1));
					
					for (Person person: people) {
						
						if(person.getEgn() == Egn) {
							inf.append("Име: " + person.getName() + "\n"  );
							if (person.getGender() == 'm') {
								inf.append("Пол: Мъжки \n");
							}else {
								inf.append("Пол: Женски \n");
							}
							inf.append("Години: " + person.getAge() + "\n" );
							inf.append("Егн: " + person.getEgn() + "\n" );
							inf.append("Тегло: " + person.getWeight() + "\n" );
							inf.append("Височина: " + person.getHeight() + "\n" );
							
							if(person instanceof Professor) {
								inf.append(String.format(person.getName() + " is ." + ((Professor)person ).getTitle()));													
							}else {
								inf.append(String.format(person.getName() + " is student at " +((Student) person).getUniversity()+ " university, with faculty Number: ." 
